...
def update_last_replied(userid):
    now = datetime.now()
...
def post_tweets(token_dict,replies,tweetids,userIDs):
    responsetweet_id=[]
    original_tweet_id=[]
    original_user_id=[]
    bot_id=[]
    ...
    now = datetime.now()
    #dt_string = now.strftime("%Y/%m/%d %H:%M:%S")
    df.to_pickle('data/replies_df.pkl') #WRITE TO PICKLE
...
def run_posting(df, timestamp, tdict):
    ...
    for i in range(NUM_THREADS):
        ...
        thread = Thread(target=post_tweets, args=(token_dict, thread_replies,thread_tweetIDs,thread_userIDs, ))
        thread.start()
