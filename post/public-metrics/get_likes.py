...
def get_favorites(consumer_key, consumer_secret, access_token, access_token_secret, user, num_tweets, since_id_dict ):
    ...
    try:
        for tweet in tweetsReq:
            tweets.append(dict(
                ...
                liked_user_id=tweet.user.id_str,
...
def get_likes(token_dict, users_list, num_tweets, since_id_dict, q):
...
def get_since_id_dict(user_file):
    save_df_path = 'data/' + user_file.split('users/')[1].split('.csv')[0] + '_likes_df.csv'
...
def get_likes_main(user_file, token_file, num_tweets):
    user_df = pd.read_csv(user_file)
    users_list = user_df['UserIDs'].apply(lambda x: str(x)).to_list()
    ...
    for i in range(NUM_THREADS):
        ...
        thread = Thread(target=get_likes, args=(token_dict, thread_users, num_tweets, since_id_dict, res_q ))
        thread.start()
...
if __name__ == '__main__':
    ...
    get_likes_main(user_file, token_file, num_tweets)
